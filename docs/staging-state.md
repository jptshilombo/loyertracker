...
| `LOYERTRACKER_TAG` | **déployé : `sha-2c5f43c7`** (Sprint 10 EP-12b US-95/96/97 + correctif RSV-S10-01, PR #173, déployé le 2026-07-04 par arbitrage PO S1, STG-ISOL-01 PASS avant/après, smoke 59/0 au 1er passage, ordre intra-jour du ledger vérifié live — candidat `PRODUCTION_READY` release `1.8.0`) |
...
> Le **`sha-9713fdaa`** (Sprint 11 EP-14a Quittances certifiées + Sprint Angular 20→22, déployé le
> 2026-07-06) est le **tag actif en staging**, avec exposition publique active sur
> `https://loyertracker.staging.loyerpro.org`. Flyway **22/22** (V22 ajoutée : `quittance`
> versionnée, `quittance_numerotation`, `quittance_verification_log`, RLS FORCE). Smoke **59/0**.
> STG-ISOL-01 PASS avant/après. Rendu PDF certifié vérifié visuellement (logo, badge DOCUMENT
> CERTIFIÉ, numéro `QT-2026-000001`, locataire anonymisé, QR + SHA-256, mentions légales).
> **Promotion Production explicitement bloquée avant la release unique `1.9.0`** (ADR-15 K5 : le
> QR pointe vers `/verify/receipt/:id`, livré au Sprint 12).
>
