...
## Décisions — kickoff clos le 2026-07-16 (PO)
...
**Décision** : la clôture est une action **manuelle** du bailleur (`POST
...
### K2 — Date de clôture effective distincte de `dateFin` — **Tranché (PO, 2026-07-16) : oui**

**Décision** : ajouter `bail.date_cloture_effective` (nullable, renseignée uniquement à la
...
### K3 — Garantie non totalement restituée à la clôture : avertissement, pas de blocage — **Tranché (PO, 2026-07-16) : avertissement**

**Décision** : la clôture n'est **pas bloquée** si `Garantie.statut ≠ RESTITUE_TOTAL` (ou
...
**Alternative écartée** : bloquer (409) tant que la garantie n'est pas `RESTITUE_TOTAL`. Écartée
par cohérence avec la doctrine « avertir sans bloquer » déjà retenue pour les doublons (BF-100,
EP-15) — point signalé comme méritant une attention particulière au kickoff (obligation
légale/financière envers le locataire sortant, contrairement à un simple doublon probable) et
**explicitement confirmé par le PO le 2026-07-16** malgré cette réserve.

### K4 — Impayés à la clôture : avertissement, pas de blocage — **Tranché (PO, 2026-07-16) : avertissement**

**Décision** : de même, la présence de paiements `IMPAYE`/`EN_RETARD`/`PARTIEL` sur le bail ne
...
### K5 — Réouverture possible (symétrie avec la restauration déjà pratiquée) — **Tranché (PO, 2026-07-16) : oui**

**Décision** : une clôture erronée peut être annulée (`Bail.statut : CLOS → ACTIF`) via un
...
### K6 — Purge de l'échéancier futur au-delà de la clôture — **Tranché (PO, 2026-07-16) : purge**

**Décision** : à la clôture, les paiements déjà générés par le batch mensuel idempotent (US-30)
...
**Alternative écartée** : conserver ces échéances telles quelles. Écartée car elles polluent la
vue impayés/alertes sans jamais devenir exigibles réellement — point signalé comme méritant
confirmation (suppression de données) et **explicitement confirmé par le PO le 2026-07-16**.
...
|---|--------|----------------------|--------|
| RSV-EP13-01 | Clôture prononcée par erreur sans que le bailleur ait restitué la garantie ou soldé les impayés (K3/K4 non bloquants, confirmés par le PO) | Avertissement explicite dans la réponse API + affichage frontend dédié ; réouverture possible (K5) | Accepté par le PO (2026-07-16) — risque résiduel assumé, mitigation à implémenter au Sprint |
| RSV-EP13-02 | Purge d'échéances `A_VENIR` (K6, confirmée par le PO) supprimant par erreur une échéance qu'un bailleur souhaitait conserver en trace | Limitée aux échéances strictement futures et non exigibles (`A_VENIR` uniquement) ; jamais de suppression d'un fait déjà survenu | Accepté par le PO (2026-07-16) — risque résiduel assumé, mitigation à implémenter au Sprint |
...
## Kickoff tranché par le PO le 2026-07-16

| # | Question | Décision | Statut |
|---|----------|----------|--------|
| K1 | Déclenchement manuel uniquement, jamais de batch automatique sur `dateFin` ? | Oui | ✅ Tranché |
| K2 | Ajouter `date_cloture_effective` distincte de `dateFin` ? | Oui | ✅ Tranché |
| K3 | Garantie non restituée à la clôture : avertissement ou blocage (409) ? | Avertissement | ✅ Tranché |
| K4 | Impayés à la clôture : avertissement ou blocage (409) ? | Avertissement | ✅ Tranché |
| K5 | Réouverture d'un bail clos par erreur : autorisée ? | Oui, sous réserve `uq_bail_actif` | ✅ Tranché |
