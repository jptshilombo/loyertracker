...
| Contrôle `STG-ISOL-01` — état **après** déploiement | ✅ **PASS** (2026-07-03, ~11:15 UTC, après déploiement + smoke + vérification garantie) — détail §10bis |
...
**Verdict `STG-ISOL-01` (état avant) : PASS.** État « après » : voir §10bis.
...
2. ~~Exécution `STG-ISOL-01` (avant/après déploiement)~~ **✅ PASS avant (§4) et PASS après
   (§10bis)**, 2026-07-03.
...
## 10bis. Contrôle `STG-ISOL-01` — état après déploiement (2026-07-03, ~11:15 UTC)

Vérification identique à l'état avant (§4), après déploiement de `sha-6a358eb6`, exécution du
smoke complet et de la vérification manuelle du cycle garantie (§9/§10) :

| Élément | État avant | État après | Conforme |
|---|---|---|---|
| Conteneurs (nombre, noms, préfixe) | 9 (8 `loyertracker-staging-*` + `nginx-proxy-manager`) | Identiques | ✅ |
| Tag actif (api/nginx) | `sha-2da27182` | `sha-6a358eb6` | ✅ (changement attendu) |
| Restart count | 0 partout | 0 partout | ✅ |
| Réseaux | `loyertracker-staging_loyertracker-net` + réseaux hôte standards | Identiques | ✅ |
| Volumes | `loyertracker-staging_postgres-data`/`..._prometheus-data` + volumes des autres projets inchangés | Identiques, aucun ajout/suppression | ✅ |
| Ports | `18080`/`18443` (staging), `80/81/443` (`nginx-proxy-manager`), aucun conflit | Identiques | ✅ |
| `nginx-proxy-manager` | `Up`, `RestartCount=0` | `Up`, `RestartCount=0`, inchangé | ✅ |
| Commande Docker globale utilisée | — | **Aucune**, sur toute la durée du Gate (déploiement, smoke, vérification garantie) | ✅ |

**Verdict `STG-ISOL-01` (état après) : PASS.** Isolation Staging confirmée intacte de bout en
bout de ce Gate — aucun impact sur les autres projets hébergés sur `ai-test-server`.
