...
* Branche active : `main` (S04 complet backend + frontend — backend via PR #11/#12/#14, frontend via PR #16 ; tests double datasource / fidelite RLS via PR #18 merge `8dbf44e` ; correctif gate Trivy openssl via PR #20 merge `98bbf41` ; synchro doc PR #19 merge `6a46b4c`)
* Derniere mise a jour : 2026-06-11
* Agent ayant mis a jour le fichier : Claude Code — CGPA Governance Officer, consignation du correctif gate Trivy / CVE-2026-45447 (PR #20)
...
* Fonctionnalites en cours :
  * smoke test runtime des flux authentifies sous le role `loyertracker_api` (stack complete Keycloak + Nginx).
* Suivi technique realise le 2026-06-10 (PR #18) : tests d'integration convertis au double datasource — application sous `loyertracker_api` (RLS `FORCE` reellement exercee, flux API authentifie inclus), Flyway en admin, harnais seed/cleanup via `JdbcTemplate` admin dedie, verrou de fidelite RLS anti-regression ; aucune modification applicative ni de migration.
...
* Blocages connus : aucun blocage technique ou de gouvernance actif. R6 (Admin API) clotûree le 2026-06-09 ; S03 (backend + frontend) clos ; S04 complet backend + frontend (PR #11 + PR #12 + PR #14 + PR #16) ; tests double datasource realises (PR #18). Tout nouveau lot (smoke test runtime, ou lot fonctionnel ulterieur) reste subordonne a un Plan d'Execution approuve.


## 6. Ou l'on va

* Prochaine phase : maintien en Phase 7 — Developpement.
* Prochaine etape recommandee : cadrer le lot suivant via un Plan d'Execution avant tout nouveau code — S04 complet et tests double datasource realises (PR #18), reste le smoke test runtime (stack complete) ou un lot fonctionnel ulterieur au choix du PO.
* Objectif du prochain Plan d'Execution : choisir et cadrer le lot suivant (smoke test runtime ou lot fonctionnel) avant toute modification.
* Priorites immediates :
  * arbitrage PO du lot suivant (smoke test runtime vs lot fonctionnel) ;
  * smoke test runtime des flux authentifies sous le role `loyertracker_api` (stack complete Keycloak + Nginx) ;
...
* Tests securite : RLS (exercee sous `loyertracker_api`, verrou de fidelite anti-regression), ReBAC, 401/403, cross-tenant/cross-affectation S02, scans secrets/dependances/images, CodeQL.
...
| Comportement runtime sous le role restreint non verifie end-to-end | Mineur | Les tests `@SpringBootTest` tournaient en superuser ; un flux applicatif dependant implicitement du superuser serait passe inapercu | Smoke test runtime 2026-06-08 (app bootee en `loyertracker_api`, TRUNCATE/DDL refuses, isolation RLS live) puis conversion double datasource le 2026-06-10 (PR #18, merge `8dbf44e`) : tous les flux API authentifies des tests d'integration tournent sous `loyertracker_api`, verrou de fidelite anti-regression, `mvn verify` 55 verts sans correctif. Reste, distinct : smoke test runtime sur la stack complete (Keycloak + Nginx) | Ferme cote tests (reste smoke test stack complete) |
...
Le suivi technique **tests double datasource (fidelite RLS, ADR-01)** est integre dans `main` via la **PR #18** (merge `8dbf44e`, CI verte 13/13 du premier coup) : les tests d'integration `@SpringBootTest` font tourner l'application sous le role restreint `loyertracker_api` (NOSUPERUSER NOBYPASSRLS) — la RLS `FORCE` est donc **reellement exercee** sur tout le chemin applicatif teste, flux API authentifie inclus — avec Flyway en admin, harnais seed/cleanup via `JdbcTemplate` admin dedie et verrou de fidelite anti-regression. `mvn verify` vert **55 tests**, aucune modification applicative ni de migration. Le risque « comportement runtime sous le role restreint » est **ferme cote tests**. Le 2026-06-11, le gate Trivy — bloque par **CVE-2026-45447** (openssl, image de base) — a ete debloque par la **PR #20** (merge `98bbf41`, `apk -U upgrade` au build du `backend/Dockerfile`), permettant le merge de la PR #19 (`6a46b4c`). Cette synchronisation consigne ce correctif ; S04 reste complet backend + frontend (PR #11/#12/#14/#16). Ne subsiste qu'un suivi technique, subordonne a un Plan d'Execution approuve : (1) smoke test runtime des flux authentifies sous `loyertracker_api` sur la **stack complete** (Keycloak + Nginx) ; (2) tout lot fonctionnel ulterieur (dashboards consolides, RGPD, OpenAPI, UX) au choix du PO. Aucun nouveau developpement ne doit demarrer sans un Plan d'Execution approuve.
