...
* Branche active : `main` (S04 complet backend + frontend — PR #11/#12/#14/#16 ; suivis techniques RLS soldes : tests double datasource PR #18 `8dbf44e`, smoke test runtime stack complete PR #22 `813b6d9` ; correctif gate Trivy PR #20 `98bbf41`)
* Derniere mise a jour : 2026-06-11
* Agent ayant mis a jour le fichier : Claude Code — CGPA Governance Officer, synchronisation post-merge smoke test stack complete (PR #22)
...
* Fonctionnalites en cours : aucune — tous les suivis techniques RLS sont soldes ; prochain lot a arbitrer par le PO.
...
Le **smoke test runtime sur la stack complete** est integre dans `main` via la **PR #22** (merge `813b6d9`, CI verte 13/13) : **46 verifications PASS / 0 FAIL** sur la stack Docker Compose reelle — JWT reels Keycloak via Nginx TLS, parcours S01->S04 complet (Admin API gestionnaire incluse), honoraires/alertes/audit, isolation cross-tenant live, pool JDBC sous `loyertracker_api` (NOSUPERUSER NOBYPASSRLS), ports internes non publies — sans **aucun ecart applicatif**. Le script rejouable est versionne (`infra/smoke/smoke-stack.sh`) et le rapport publie (`docs/cgpa/07-devsecops/rapport-smoke-test-stack-complete.md`). Avec la conversion double datasource (PR #18), **tous les suivis techniques de la remediation RLS (ADR-01) sont soldes** : le risque « comportement runtime sous le role restreint » est **Ferme**. S04 reste complet backend + frontend (PR #11/#12/#14/#16). Prochaine etape : **arbitrage PO du lot suivant** — lot fonctionnel (dashboards consolides, RGPD, OpenAPI, UX) ou production readiness (staging/CD/backup/observabilite, axe Exploitation 1/4) — a cadrer par un Plan d'Execution approuve avant tout nouveau code.
