...
> Reprise CGPA v5.0.1 du 2026-06-13 : `Resume Approved with Reservations`. Projet repris en Phase 7 — Développement, plan Production Readiness lot 4/4 comme prochain incrément. Migration additive depuis la discipline v3.0.1 (Project State File) : phase courante, gates validés, décisions, risques, backlog et historiques conservés. Réserves non bloquantes tracées en §13 (R-3 décision registry/CD/backup, R-4 jalon Staging v4.0).
...
* Branche active : `main` (S04 complet backend + frontend — PR #11/#12/#14/#16 ; suivis techniques RLS soldes : tests double datasource PR #18 `8dbf44e`, smoke test runtime stack complete PR #22 `813b6d9` ; correctif gate Trivy PR #20 `98bbf41` ; **plan Production Readiness lots 1-3 livres** : CD/GHCR PR #24 `321e195`, staging durci PR #25 `2403e34`, backup/restore PR #26 `1ed1080`)
* Derniere mise a jour : 2026-06-13
* Agent ayant mis a jour le fichier : Claude Code — CGPA Chief Delivery Officer, reprise CGPA v5.0.1 + resynchronisation Production Readiness lots 1-3 (PR #24/#25/#26)
...
Le socle technique est operationnel ou tres avance : backend Spring Boot, frontend Angular, Keycloak, PostgreSQL, Flyway, RLS, Nginx, Docker Compose, CI GitHub Actions, CodeQL, lint, scans de securite. Les phases de cadrage et DevSecOps sont validees jusqu'au Gate 06. Le projet est en Phase 7 Developpement. S01 est cloture documentairement. S02 est realise en backend pour US-20 a US-24 et dispose d'un frontend minimal bailleur/gestionnaire pour exploiter biens, baux et affectations. S03 (paiements & garanties, US-30/31/32) est realise (backend + frontend) et clos. S04 backend est complet : lot 1 honoraires (US-40) integre via la PR #11, lot 2 alertes & consultation d'audit (US-50/51/52, US-62) integre via la PR #12, et l'alerte PREAVIS (US-50, EF-62) — critere d'acceptation fige par le PO (bande de preavis a 90 jours, exclusive de FIN_BAIL) — integree via la PR #14. Le reste fonctionnel backend S04 est ainsi clos. Le frontend S04 (consoles honoraires, alertes et journal d'audit, bailleur et gestionnaire) a ete integre via la PR #16 : S04 est desormais complet backend + frontend. Le suivi technique « double datasource » a ete realise via la PR #18 : les tests d'integration `@SpringBootTest` font desormais tourner l'application sous le role restreint `loyertracker_api` (NOSUPERUSER NOBYPASSRLS), donc la RLS `FORCE` est reellement exercee sur tout le chemin applicatif teste (flux API authentifie inclus), sans aucune modification applicative ni de migration. Le smoke test runtime sur la stack complete (Nginx TLS, Keycloak 24, JWT reels, Admin API, parcours S01->S04, isolation cross-tenant live) a ete realise via la PR #22 : 46 verifications PASS / 0 FAIL, aucun ecart applicatif revele, script rejouable versionne (`infra/smoke/smoke-stack.sh`). Tous les suivis techniques de la remediation RLS sont soldes. Le PO a ensuite arbitre en faveur du lot **production readiness**, cadre comme un plan en 4 lots (Niveau 3, defauts A-D approuves). Lots 1 a 3 livres et integres dans `main` : lot 1 chaine de livraison vers GHCR + scan Trivy de l'image web (PR #24, merge `321e195`) ; lot 2 stack staging durcie — Nginx/web non-root, SPA reelle, compose staging sur images GHCR (PR #25, merge `2403e34`) ; lot 3 sauvegarde/restauration PostgreSQL versionnee, prouvee par un drill de restauration reel (PR #26, merge `1ed1080`). **Lot 4/4 restant** : observabilite minimale, runbook d'exploitation, installation du cron de backup sur l'hote, deploiement staging reel + smoke test contre staging — subordonne a un Plan d'Execution approuve. Le 2026-06-13, la **reprise CGPA v5.0.1** a statue `Resume Approved with Reservations` (continuite saine, aucun gate rejoue ; reserves non bloquantes : decision registry/CD/backup a journaliser, jalon Staging v4.0 a ouvrir au lot 4).
...
* Fonctionnalites en cours : plan Production Readiness — lots 1-3 livres (PR #24/#25/#26), **lot 4/4 a cadrer** (observabilite, runbook, cron backup hote, deploiement + smoke staging).
...
* Dette technique :
  ...
  * observabilite limitee aux logs/Actuator, sans metriques ni alerting (a traiter lot 4) ;
  * cron de backup non installe sur l'hote et runbook d'exploitation absent (a traiter lot 4) ;
  * deploiement staging reel et smoke contre staging non encore realises (lot 4) ;
  ...
  * rollback formel : par tag d'image immuable `sha-<8>` (lot 1) ; procedure d'exploitation a documenter (runbook lot 4).
* Dette documentaire :
...
* Blocages connus : aucun blocage technique ou de gouvernance actif. R6 (Admin API) clotûree le 2026-06-09 ; S03 (backend + frontend) clos ; S04 complet backend + frontend (PR #11 + PR #12 + PR #14 + PR #16) ; tests double datasource realises (PR #18) ; smoke test stack complete realise (PR #22) ; Production Readiness lots 1-3 livres (PR #24/#25/#26). Le lot 4 (observabilite, runbook, cron, deploiement + smoke staging) reste subordonne a un Plan d'Execution approuve.


## 6. Ou l'on va

* Prochaine phase : maintien en Phase 7 — Developpement.
* Prochaine etape recommandee : cadrer le lot suivant via un Plan d'Execution avant tout nouveau code — S04 complet et tous les suivis techniques RLS soldes (PR #18 + PR #22), le choix du lot (fonctionnel : dashboards consolides, RGPD, OpenAPI, UX ; ou production readiness) appartient au PO.
* Objectif du prochain Plan d'Execution : choisir et cadrer le lot suivant avant toute modification.
* Priorites immediates :
  * arbitrage PO du lot suivant (lot fonctionnel vs production readiness) ;
...
* Observabilite : logs JSON ECS backend, access logs JSON Nginx, Actuator health/info ; pas encore de metriques ni alerting (observabilite minimale a livrer au lot 4).
* Sauvegarde/restauration : scripts versionnes `infra/backup/backup-postgres.sh` (dump `pg_dump -Fc` base complete app + Keycloak, `pg_dumpall --globals-only` pour les roles cluster, verification d'integrite `pg_restore --list`, retention 7 quotidiennes + 4 hebdomadaires, fichiers `600`/repertoire `700` hors depot) et `infra/backup/restore-postgres.sh` (restauration orchestree, destructive a confirmation, drill de restauration reel execute — RPO 24 h / RTO < 1 h) — lot 3, PR #26. Installation du cron sur l'hote reportee au runbook lot 4.
* Staging : `docker-compose.staging.yml` durci (web/Nginx non-root, SPA reelle, images consommees depuis GHCR) — lot 2, PR #25. Deploiement staging reel et smoke contre staging non encore realises (lot 4 ; jalon Gate Staging Readiness CGPA v4.0 a ouvrir).
...
* Valeur metier : 3/4
* Score global : 23/32
* Commentaire : niveau Partiel avance. Le socle couvre les lots metier EP-03 (S02), EP-04 (S03 paiements/garanties) et EP-05 complet backend + frontend (S04 honoraires, alertes dont PREAVIS, audit), testes et integres. L'axe Exploitation passe de 1/4 a **2/4** avec le plan Production Readiness lots 1-3 : chaine de livraison CD vers GHCR (tags immuables `sha-<8>` + scan Trivy web, PR #24), stack staging durcie non-root sur images GHCR (PR #25), sauvegarde/restauration PostgreSQL prouvee par un drill reel (PR #26). Restent pour atteindre 3/4 (lot 4) : observabilite minimale (metriques/alerting), runbook d'exploitation, cron de backup installe sur l'hote, **deploiement staging reel + smoke contre staging** et prononce du Gate Staging Readiness. Le projet n'est pas encore deploye en staging/prod et conserve la discipline CGPA : plan approuve avant tout nouveau lot.
...
| Pas de staging/prod/CD/backup | Majeur | Non readiness production | Plan Production Readiness en 4 lots : CD/GHCR (PR #24), staging durci (PR #25), backup/restore + drill reel (PR #26) livres ; restent observabilite, runbook, cron hote, deploiement + smoke staging (lot 4) | En reduction |
...
| Jalon Staging CGPA v4.0 non ouvert : `staging-state.md` absent, Gate Staging Readiness non prononce (R-4) | Mineur gouvernance | Obligation v4.0 non encore instanciee (legitime tant que staging non deploye) | Ouvrir `staging-state.md` et prononcer le Gate Staging au lot 4 (deploiement staging reel) | Ouvert (lot 4) |
...
Le PO a arbitre le lot suivant en faveur du **plan Production Readiness en 4 lots** (axe Exploitation). **Lots 1 a 3 livres et integres dans `main`** : lot 1 chaine de livraison CD vers GHCR + scan Trivy de l'image web (PR #24, merge `321e195`) ; lot 2 stack staging durcie non-root sur images GHCR (PR #25, merge `2403e34`) ; lot 3 sauvegarde/restauration PostgreSQL versionnee prouvee par un drill de restauration reel — RPO 24 h / RTO < 1 h (PR #26, merge `1ed1080`). La reprise CGPA v5.0.1 du 2026-06-13 a statue `Resume Approved with Reservations` et resynchronise ce Project State avec les lots 1-3 (bloc `framework:` v5.0.1 ajoute). Le score Exploitation passe a 2/4 (global 23/32).

**Prochaine etape : lot 4/4 du plan Production Readiness**, subordonne a un **Plan d'Execution approuve par le PO avant tout nouveau code** (verrou CGPA v3.0). Perimetre cadre : observabilite minimale (metriques/alerting) ; runbook d'exploitation (deploiement, rollback par tag d'image, sauvegarde/restauration, incidents) ; installation du cron de backup sur l'hote ; **deploiement staging reel + smoke test contre staging** ; ouverture de `staging-state.md` et prononce du **Gate Staging Readiness** (CGPA v4.0, reserve R-4). Apres execution : re-synchroniser le Project State et statuer le Gate Staging.
