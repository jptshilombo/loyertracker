...
* Branche active : `main` (S04 complet backend + frontend — backend via PR #11/#12/#14, frontend via PR #16 ; tests d'integration convertis au double datasource / fidelite RLS via PR #18 merge `8dbf44e`)
* Derniere mise a jour : 2026-06-11
* Agent ayant mis a jour le fichier : Claude Code — CGPA Governance Officer, synchronisation post-merge double datasource (PR #18)
...
Le suivi technique **tests double datasource (fidelite RLS, ADR-01)** est integre dans `main` via la **PR #18** (merge `8dbf44e`, CI verte 13/13 du premier coup) : les tests d'integration `@SpringBootTest` font tourner l'application sous le role restreint `loyertracker_api` (NOSUPERUSER NOBYPASSRLS) — la RLS `FORCE` est donc **reellement exercee** sur tout le chemin applicatif teste, flux API authentifie inclus — avec Flyway en admin, harnais seed/cleanup via `JdbcTemplate` admin dedie et verrou de fidelite anti-regression. `mvn verify` vert **55 tests**, aucune modification applicative ni de migration. Le risque « comportement runtime sous le role restreint » est **ferme cote tests**. Cette synchronisation post-merge consigne ce suivi ; S04 reste complet backend + frontend (PR #11/#12/#14/#16). Ne subsiste qu'un suivi technique, subordonne a un Plan d'Execution approuve : (1) smoke test runtime des flux authentifies sous `loyertracker_api` sur la **stack complete** (Keycloak + Nginx) ; (2) tout lot fonctionnel ulterieur (dashboards consolides, RGPD, OpenAPI, UX) au choix du PO. Aucun nouveau developpement ne doit demarrer sans un Plan d'Execution approuve.
